...
.. csv-table:: NAM_BACKUP content
   ...
   "XBAK_TIME","REAL(:,:)",|XNEGUNDEF|
   "NBAK_STEP","INTEGER(:,:)",|NNEGUNDEF|
   "XBAK_TIME_FREQ","REAL(:)",|XNEGUNDEF|
   "XBAK_TIME_FREQ_FIRST","REAL(:)",|XNEGUNDEF|
   "NBAK_STEP_FREQ","INTEGER(:)",|NNEGUNDEF|
   "NBAK_STEP_FREQ_FIRST","INTEGER(:)",|NNEGUNDEF|
...
* :code:`LBAK_COMPRESS(m)` : enable lossless compression of data for each model m. This can have a negative impact on performance. This option loses precedence over LIO_COMPRESS of :ref:`nam_confio`.

* :code:`LBAK_COMPRESS_LEVEL(m)` : set the compression level for each model m. The value must be in the 0 to 9 interval (0 for no compression, 9 for maximum compression). This option loses precedence over LIO_COMPRESS_LEVEL of :ref:`nam_confio` if  LIO_COMPRESS=.TRUE.
