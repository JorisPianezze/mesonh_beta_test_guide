...
Pourquoi vaut-il mieux utiliser cet outil que l'autre simulateur radar inclus dans les diagnostics de Méso-NH ?
  ...
  Les besoins de cet opérateur d’observation radar pour l’évaluation de simulations AROME également, ont conduit à privilégier le développement d’une version offline pour la recherche compatible à la fois avec AROME et Méso-NH, et donc plus facile à maintenir et à faire évoluer de manière collaborative (cf. le `dépôt git d'operadar <https://github.com/UMR-CNRM/operadar>`_)
